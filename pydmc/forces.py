    ...
    def compute_forces(self, steps_per_block):
        ...
        for iw in range(len(self._weights)):
            for i, da in enumerate(self._increments):
                ...
                tderiv = (np.array(self._ts[i+1][iw]) - np.array(self._ts[0][iw]))/da
                ...
                tderiv_partial_sums = np.zeros(len(tderiv)-steps_per_block)
                ...
                fpulay = -(np.array(self._local_es[0][iw][steps_per_block:] - energy) \
                    * (tderiv_partial_sums + sderiv_partial_sums + jderiv[steps_per_block:])) # + psi_logderiv[:-steps_per_block]))
